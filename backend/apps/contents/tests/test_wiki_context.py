...
class TestWikiContextFiltering:
    ...
    def test_list_wikis_filtered_by_chapter(self):
        ...
        wiki_a = baker.make("contents.WikiEntry", branch=branch, name="Wiki A", first_appearance=1)

        # Wiki B: first_appearance=10
        wiki_b = baker.make("contents.WikiEntry", branch=branch, name="Wiki B", first_appearance=10)

        # Wiki C: first_appearance=None (Always visible)
        wiki_c = baker.make(
            "contents.WikiEntry", branch=branch, name="Wiki C", first_appearance=None
        )
        ...
        assert wiki_a.id in result_ids_ch1
        assert wiki_b.id not in result_ids_ch1
        assert wiki_c.id in result_ids_ch1
        ...
        assert wiki_a.id in result_ids_ch10
        assert wiki_b.id in result_ids_ch10
        assert wiki_c.id in result_ids_ch10
        ...
        assert wiki_a.id in result_ids_ch5
        assert wiki_b.id not in result_ids_ch5
        assert wiki_c.id in result_ids_ch5

    def test_list_wikis_without_chapter_param(self):
        """current_chapter 파라미터가 없으면 모든 위키 반환"""
        branch = baker.make("novels.Branch")

        wiki_a = baker.make("contents.WikiEntry", branch=branch, first_appearance=1)
        wiki_b = baker.make("contents.WikiEntry", branch=branch, first_appearance=10)
    ...
    def test_list_wikis_future_chapter_is_hidden(self):
        ...
        wiki = baker.make("contents.WikiEntry", branch=branch, first_appearance=10)
