...
class TestWikiEntryViewSet:
    ...
    def test_list_wiki_entries(self) -> None:
        """위키 목록 조회"""
    ...
    def test_create_wiki_entry_with_initial_content(self) -> None:
        """초기 콘텐츠와 함께 위키 생성"""
        url = f"/api/v1/branches/{self.branch.id}/wikis/"
        data = {
            "name": "캐릭터",
    ...
    def test_create_wiki_unauthenticated(self) -> None:
        """비인증 사용자 위키 생성 불가"""
    ...
    def test_retrieve_wiki_with_context(self) -> None:
        """문맥 인식 위키 조회"""
        wiki = baker.make("contents.WikiEntry", branch=self.branch, name="캐릭터")
        baker.make(
