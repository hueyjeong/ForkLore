...
class TestReadingHistoryView:
    ...
    def test_reading_history_requires_auth(self):
        """인증 없이 접근 불가"""
        client = APIClient()
    ...
    def test_reading_history_returns_user_logs(self):
        """읽은 기록 목록 반환"""
    ...
    def test_reading_history_excludes_other_users(self):
        """다른 사용자 기록 제외"""
...
class TestContinueReadingView:
    ...
    def test_continue_reading_requires_auth(self):
        """인증 없이 접근 불가"""
        client = APIClient()
    ...
    def test_continue_reading_returns_chapter(self):
        """이어보기 정보 반환"""
        user = baker.make("users.User")
        branch = baker.make("novels.Branch")
    ...
    def test_continue_reading_no_history(self):
        """기록 없으면 첫 회차"""
        user = baker.make("users.User")
        branch = baker.make("novels.Branch")
        chapter1 = baker.make("contents.Chapter", branch=branch, chapter_number=1)
...
class TestBookmarkListView:
    ...
    def test_bookmarks_returns_user_bookmarks(self):
        """북마크 목록 반환"""
        user = baker.make("users.User")
        chapter = baker.make("contents.Chapter")
